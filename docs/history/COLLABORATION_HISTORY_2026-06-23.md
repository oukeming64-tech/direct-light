...
**当前发布基线：v0.9.0 已发布（网页 + macOS 桌面，用户确认 2026-06-23）**。公开仓库为 https://github.com/oukeming64-tech/direct-light，GitHub Pages 自动部署和 Tauri macOS Release CI 均已落地；`src-tauri/Cargo.lock` 已提交，桌面构建使用锁文件保持可复现。v0.9 自定义灯具预设导入/导出已完成并通过用户真机验收，TopBar / `package.json` / `tauri.conf.json` / `Cargo.toml` 均为 `0.9.0`，tag 为 `v0.9.0`。个人复盘 `个人重构/` 继续保留本地、不入仓库。**当前在做：v0.10 多语言 UI（规格 `V0_10_I18N_SPEC.md`）——i18n 基础设施（用户称 v0.10.1a）+ v0.10b 全部 tier-A 核心 UI 抽取均已通过用户视觉验收（2026-06-23）；下一步 v0.10.1（内置数据派生显示标签 + `sceneDiff` 插值文案，需 Codex 英日 copy）与 v0.10c/d 收口；TopBar 版本仍 `v0.9.0`，按纪律到 v0.10d closeout 才升 `v0.10.0`。**
...
**v0.8.0 已发布（网页 + macOS 桌面），并把桌面构建改成可复现（Claude Code，2026-06-22）**：用户验收通过后 push 到 `main`，版本号统一升到 `0.8.0`（`TopBar`/`package.json`/`tauri.conf.json`/`Cargo.toml`）。① 网页：push `main` 触发 GitHub Pages 部署成功，在线 demo 已更新。② 桌面：push `v0.8.0` tag 触发 `release.yml`，**首次构建失败**——上游 `time` 0.3.50（当天发布）编译报 `unresolved import time_macros::timestamp`；根因是仓库**没有提交 `src-tauri/Cargo.lock`**，每次 tag 构建都重新解析依赖，于是拉到了刚发布的坏版本（v0.7.1 当时碰巧解析到好版本才过）。先在 `release.yml` 加一步 `cargo generate-lockfile` + `cargo update -p time --precise 0.3.49` 救急，把 `v0.8.0` tag 移到修复 commit 重新构建，**通用 macOS `.dmg` 构建成功（5m57s）并已发布**（草稿→用户授权后 `gh release edit --draft=false` 发布）。③ **长期修复**：新增 `.github/workflows/lockfile.yml`（`workflow_dispatch`，ubuntu 上 `generate-lockfile` + 固定 `time` 0.3.49 后把 `src-tauri/Cargo.lock` 提交回仓库），已跑一次并提交 4762 行的 `Cargo.lock`（`time` 锁到 0.3.49）；随后把 `release.yml` 的临时 pin 步骤删掉，改为直接用已提交的 `Cargo.lock` 构建，构建从此可复现、不再随上游 patch 漂移；需要刷新锁文件时手动跑 “Update Cargo.lock” 工作流即可。是否进一步加 `--locked` 严格校验留作可选硬化。④ `lockfile.yml` 里的 `time` 0.3.49 是**临时 pin**（已加注释说明）：上游修好出 `0.3.51+` 后，把那行 `--precise` 改版本或删掉，`time` 即可跟其它依赖一起前进；其它 crate 不受此 pin 影响。提交的锁是时间点快照，不会自己变旧、也不会自动升级，发版前刷一次锁即可拿到当下最新可兼容版本。

**v0.9a 自定义灯具 schema + 纯函数已落地（Claude Code 主导，2026-06-22；后续 b/c/d 已完成）**：开始 v0.9 自定义灯具导入/导出。Codex 把 v0.9 拆为 a/b/c/d 并指定 **Claude 主导**（涉及大量校验/计算函数），简单机械段后续可交 OpenRouter。新增 `V0_9_CUSTOM_FIXTURE_SPEC.md`（唯一规格，详述 v0.9a、勾勒 b/c/d）。与用户敲定：**录入方式 = 导入导出 + 存当前灯**；**字段范围 = 对齐内置完整字段**（自定义器械与内置 `FixturePreset` 同形，可直接进同一下拉/`applyFixturePreset`/能力标签/A-B）。本刀只做数据结构 + 单条器械纯函数，**不动 store/UI/localStorage/渲染**。① 类型 `src/types.ts`：把 `FixturePreset` 类型从 `data/fixturePresets.ts` 上移到此（单一真源，`fixturePresets.ts` 改为 `import type { FixturePreset } from '../types'` 并 re-export，旧调用不破），加 `FixtureSource`（`'builtin'|'custom'`）、`FixturePreset.source?`、`CustomFixturePreset`（= `FixturePreset & { source:'custom'; createdAt; updatedAt }`）、`CustomFixturePack`（导入/导出 envelope：`schema:'direct-light/custom-fixtures'` + `version` + `fixtures[]`，一个 pack 装 1..N）。② 纯函数 `src/domain/customFixtures.ts`（无 React/store/localStorage/`Date.now()`，时间戳由调用方传入）：`normalizeCustomFixture(input, {fallbackId, now})` 把不可信值校验/钳制/补默认成 `CustomFixturePreset`（硬错误：非对象/缺 label/缺 directLightDefaults → 拒绝；软问题：枚举回退、数值钳制、颜色回退 → warning 仍保留；`directLightDefaults` 钳制区间 = UI 滑杆区间，故 8 个内置无警告通过、种进灯后仍可滑），`buildCustomFixtureFromLight`（「存当前灯为器械」，带 baseFixture 时沿用真实元数据），`getAllFixtures`/`findFixtureById`（内置在前、自定义在后）、`isCustomFixture`/`isReservedFixtureId`、`CUSTOM_FIXTURE_SCHEMA`/`_VERSION`/`_RANGES` 常量；自定义 id 一律 `custom-` 前缀，永不撞内置。**a/b 边界**：v0.9a = 单条器械数据结构 + 纯归一；v0.9b = 文件级（`JSON.parse` + envelope + 多条遍历/去重/聚合 errors-warnings + 序列化导出 + 用户文案）。`npx tsc -b`/`npm run lint`/`npm run build` 全绿；确定性自测 86/0（esbuild 不可用，改用 `tsc → CJS` 跑断言：内置全部无警告归一且 directLightDefaults 不变、硬错误、钳制、枚举/颜色回退、id 撞内置改写、`buildFromLight` 映射钳制、normalize 幂等、merge/lookup/guard）。临时自测文件已删。TopBar 后来在 v0.9d 升到 `v0.9.0`。

**v0.9b 文件级导入/导出/校验已落地（OpenRouter glm-5.2 起草、Claude Code 逐行审核集成，2026-06-22；后续 c/d 已完成）**：按规格 §7，在 v0.9a 纯函数之上做 pack 级文件读写。新增 `src/domain/customFixturePack.ts`（纯函数，string 入 / 结构化出，无 UI/IO/`Date.now()`）：`parseCustomFixturePack(text, {now, takenIds?})` → `{fixtures, errors, warnings}`，容三种文件形态（完整信封 `{schema,version,fixtures}` / 裸器械数组 / 单个器械对象，后两者给提示性 warning），逐条调 `normalizeCustomFixture` 并把错误/警告加 `第 N 条：` 前缀、坏条目跳过好条目保留，id 在 pack 内 + 对 `takenIds` 去重（撞了改 `-2/-3…` 并 warn），`JSON.parse` 失败/无法识别结构/无器械分别给明确中文文案，**永不抛**；`serializeCustomFixturePack(fixtures, {now})` 产出 `schema/version/exportedAt/fixtures` 的 2 空格美化 JSON。**a/b 落点**：a = 单条数据结构 + 纯归一，b = 文件信封 + 多条聚合 + 序列化，互不重叠。分工：glm-5.2 按 Claude 的精确规格起草，Claude 审核（去掉多余 `as` 断言因 `CUSTOM_FIXTURE_SCHEMA` 本是字面量类型、统一单引号无分号、模板字符串 fallbackId）后落盘。`npx tsc -b`/`npm run lint`/`npm run build` 全绿；确定性自测 19/0（同样 `tsc → CJS` 跑断言：round-trip id/directLightDefaults 稳定、非法 JSON、裸数组、单器械、未识别结构、空 fixtures、坏条目跳过且 `第 N 条` 前缀、越界警告前缀、pack 内 id 去重、`takenIds` 改名、schema/version 警告）。**踩坑记**：自测里 `(o)=>({...})` 箭头返回括号对象在这台机的 tsc(~6.0) CJS 转译路径下被误解析成函数类型（剥掉对象值），改成 `function`+`return` 绕开——仅自测脚手架问题，不影响产物。临时自测文件已删。

**v0.9c 接器械下拉 + store + 本地持久化已落地（Claude Code 主导，2026-06-23；v0.9d 已完成并验收）**：按规格把 v0.9a/b 接进真实应用，自定义器械与内置完全互通。① 持久化 `src/lib/storage.ts`：新增 `loadCustomFixtures`/`saveCustomFixtures`（localStorage key `direct-light.customFixtures.v1`，与方案的 `presets.v1` 分开；只写已归一的 `CustomFixturePreset`）。② store：`storeTypes.ts` 加 `customFixtures: CustomFixturePreset[]` 状态 + 4 个动作；`store.ts` 初始化 `loadCustomFixtures()` 并装配 `createFixtureActions(set,get)`。③ 新动作 `src/state/actions/fixtureActions.ts`：`saveCurrentLightAsFixture(lightId,name)`（`buildCustomFixtureFromLight` 快照当前灯，带 baseFixture 时沿用真实元数据，存库并把该灯指向新器械）、`removeCustomFixture(id)`（删库 + 把引用它的灯 `fixturePresetId` 清空、原始参数不动；未知 id no-op）、`importCustomFixtures(text)`（`parseCustomFixturePack` + 已存 id 作 `takenIds` 去重，返回 `{added,errors,warnings}`）、`exportCustomFixtures()`（`serializeCustomFixturePack` 全库）。④ `applyFixturePreset` 从 `FIXTURE_PRESETS.find` 改为 `findFixtureById(id, s.customFixtures)`，内置 + 自定义统一解析（行为对内置零变化）。⑤ UI：`LightBaseSection` 器械下拉拆成「内置器械 / 我的器械」两个 optgroup，选中自定义时加「自定义」紫徽标；新增 `LightFixtureActions.tsx`（存当前灯为器械 / 导入… / 导出 / 删除此器械，`window.prompt` 取名、`<input type=file>`+`File.text()` 读文件、`downloadDataUrl` 下 JSON、小状态行回显导入条数/提示/错误，明细打 `console.info`）；`LightPanel` 改用 `findFixtureById` 解析当前 fixture 并把 `customFixtures` 传给 Base section。⑥ 兼容：`customFixtures` 是独立 store slice、不进 `SceneConfig`，旧方案引用已删器械时 `findFixtureById`→undefined 自动回退「自定义参数」，`sceneMigration` 无需改；Codex 复核又补齐下拉 value 的显式兜底。分工：本刀碰 store/UI，Claude Code 自己写（非机械段，未外包）。`npx tsc -b`/`npm run lint`/`npm run build` 全绿；纯逻辑已在 v0.9a/b 单测覆盖，store 包装层经 tsc 校验，下拉/导入导出/持久化交用户视觉验收（按《工作约定》不驱动 R3F 预览）。TopBar 已在 v0.9d 升到 `v0.9.0`，README/CHANGELOG 也已改为已支持。

**v0.9d 收口已完成，v0.9 通过用户视觉验收并发布（Claude Code，Codex 复核，2026-06-23）**：v0.9c 全部 8 步用户肉眼验收通过（下拉分组、存当前灯为器械、切换种参、导出/导入、删除回退、刷新持久化、坏 json 不崩）。收口：① **携带验证**——保存方案 `structuredClone(scene)` + `normalizeLight` 的 `...light` 保留 `fixturePresetId`；复制灯 `...structuredClone(src)` 保留；A/B 用 scene 克隆携带、`sceneDiff` 无 fixture 引用（按 spec §8，器械不同时 type/intensity 等已让「灯光」chip 高亮，无需新增 diff 类别）；旧方案引用已删自定义器械时 `findFixtureById`→undefined 自动回退「自定义参数」、不崩，`sceneMigration` 无需改；Codex 复核补一处 UI 兜底，让器械下拉在 resolved fixture 缺失时也显式回到「自定义参数」。② TopBar 版本 `v0.8.0 → v0.9.0`；发布后 `package.json`/`tauri.conf.json`/`Cargo.toml` 也同步到 `0.9.0`，tag 为 `v0.9.0`。③ 全套文档同步到「v0.9 完成」（COLLABORATION/V0_9 spec/ROADMAP/CLAUDE/AGENTS/ARCHITECTURE/CHANGELOG/README*/CONTRIBUTING/HERMES）。`tsc·lint·build` 全绿。**v0.9 自定义灯具导入/导出全线完成；后续主线：v0.10 多语言 UI。** 分工说明：本刀主要是验证 + 版本号 + 文档；用户已重申「能甩 OpenRouter 就甩」，后续凡有可规格化的代码草稿默认交 OpenRouter 起草、Claude 审核集成。

**v0.10 多语言 UI 口径已由 Codex 定稿（规格定稿，2026-06-23；实现进度见下方 v0.10.1a / v0.10b 各批记录）**：新增 `V0_10_I18N_SPEC.md`，把多语言明确拆为 v0.10a-d。核心口径：语言是 app preference，不进入 `SceneConfig`、保存方案、A/B 快照或自定义器械 JSON；默认 `zh-CN`，首批 `zh-CN` / `en` / `ja`；用项目内轻量 typed dictionary，不先引入重型 i18n 依赖；用户输入、品牌/型号、单位、id 不翻译；内置器械/道具/附件/机位等可通过 id/语义字段做本地化显示 helper，但原始数据表不改写。版本拆分：v0.10a i18n 基础设施 + 顶栏语言入口；v0.10b 核心 UI 抽取；v0.10c 内置显示标签 + 英日 copy pass + raw Chinese 扫描；v0.10d 语言切换不改数据、布局不溢出、用户视觉验收和文档收口。分工：Codex 审规格和 copy，Claude Code 主实现，OpenRouter 可起草机械抽字串，Hermes 只做窄范围扫描/术语表草稿。**执行路线按风险重切**（2026-06-23 与用户敲定，见记忆 `v0-10-i18n-route`）：把全仓约 398 个带引号中文串分三档——A 纯 UI chrome、B 内置数据表显示标签（`LIGHT_TYPE_LABELS`/`preset.label`/`object.kind`/capability/材质名/机位名）、C `sceneDiff` 插值文案；**a-light → b（只做 A 档）→ 把 B+C 合并推迟到 v0.10.1**。

**v0.10.1a i18n 基础设施已落地并通过用户视觉验收（Claude Code，2026-06-23）**：按规格 §3/§7 与重切路线做「a-light」地基切片，唯一对外行为是新增语言菜单 + 顶栏/视角文案三语化，**不碰任何数据表**。① 新增 i18n 模块 `src/i18n/`：`languages.ts`（`AppLanguage = 'zh-CN'|'en'|'ja'`、`DEFAULT_LANGUAGE`、`SUPPORTED_LANGUAGES`、`isAppLanguage`）；`index.ts`（纯 `t(language,key,params?)`：缺键回退 zh-CN、查无渲染 `⟦key⟧`、`{name}` 插值留未知占位）；`useT.ts`（从 store 读 `language`、按语言 memoize 的 hook）；**词条目录按域拆分**（扩展性优先）`messages/`：`topBar`/`views`/`common`/`objectList`/`presetBar`/`paramPanel` 各自持有 zh 真源（`as const`→key 集）+ en/ja 用 `Record<Key,string>` 强制补齐，`messages/index.ts` 聚合并从合并后的 zh 派生 `MessageKey`——加 key = 改一个域文件 + 注册三处 spread，加语言 = 各域加一份 dict + 一处 spread，编译器盯着任一域 en/ja 漂移。② store：`storeTypes.ts` 顶层加 `language: AppLanguage` + `setLanguage`（**不进 `scene`**）；`viewActions.ts` 的 `setLanguage` 先 `saveLanguage` 再 `set({language})`（浅合并保各分片引用不变）；`store.ts` 初始 `loadLanguage()`。③ 持久化 `storage.ts`：独立 key `direct-light.language.v1` 的 `loadLanguage`/`saveLanguage`（未知/缺失→`zh-CN`）。④ UI：新增 `LanguageMenu.tsx`（顶栏紧凑 `<select>`，三语各以自身文字书写）；`TopBar` 副标题/5 视角标签/导出·重置三语化（保留 `v0.9.0` 字样与 `DEBUG_PRESETS` 导演预设按钮的中文——后者属 B 档数据记录）；`ViewBadge` 提示语走 `t(\`view.hint.${viewMode}\`)`，`canvasLayout.ts` 删掉旧 `VIEW_HINTS` 常量。`npx tsc -b`/`npm run lint`/`npm run build`/`git diff --check` 全绿；**确定性边界自测 21/0**（`tsc → CJS` 跑断言，临时文件已删）：`setLanguage` 在 `en`→`ja`→`zh-CN` 切换前后，`scene`/`customFixtures`/`presets`/`compareB` **引用全等**（规格 §9 的「切语言不改数据」证明），默认 `zh-CN`，翻译器真键/缺键标记/插值均正确。按《工作约定》语言菜单/持久化交用户视觉验收，已通过。

**v0.10b 核心 UI 抽取第一批（对象列表 + PresetBar + ParamPanel 壳）已落地（Claude Code，2026-06-23；待用户验收）**：在 a-light 的可扩展 `messages/` 上接着抽 tier-A 纯 chrome。转 `useT` 的文件：`object-list/{StudioSection,PeopleSection,LightsSection,CameraSection,SceneObjectsSection}.tsx`（区段标题、添加/复制/删除/开关/显隐 tooltip、满额徽标、加道具下拉占位与控光器材 optgroup）、`PresetBar.tsx`（方案名占位、保存方案、空态提示、无预览、载入/复制/删除 tooltip）、`ParamPanel.tsx` 空态两行。新增域文件 `messages/{objectList,presetBar,paramPanel}.ts` + `common.ts`（`common.full = 满 {max}`、`language.label`）。**严守数据边界**：`LIGHT_TYPE_LABELS[light.type]`、场景物体 `preset.label`、`object.kind`、用户输入名（person/light/object/preset 名）一律留原样、归 v0.10.1 的 B/C 档。这 7 个文件已无残留中文字面量；`tsc·lint·build` 全绿。

**v0.10b 第二批（4 个独立面板）已落地（Claude Code，2026-06-23；待用户验收）**：`StudioPanel`/`CameraPanel`/`PersonPanel`/`ObjectPanel` 转 `useT`。新增域文件 `messages/{studioPanel,cameraPanel,personPanel,objectPanel}.ts`，并在 `common.ts` 加入人/物共享的几何轴标签（`common.axisX/axisYoffGround/axisZ/facing`）。**分工按《工作约定》**：key 命名/数据 vs chrome 分类/zh 真源/集成由 Claude 定，en+ja 翻译草稿交 **OpenRouter glm-5.2** 生成（两次调用，约 90 串×2 语），Claude 审校（修了日文混入中文「等效→等価」「面部→顔」、`头·俯仰→頭·上下`，英文统一句式大小写）后落盘。**数据边界**：pose 预设 `pp.label`、承载面 `surface.label`、机位预设 `preset.label`、材质 `SCENE_OBJECT_MATERIALS[k].label`、`obj.kind`、用户名一律留原样，归 v0.10.1。`CameraPanel` 极坐标数学、`PersonPanel` pose 逻辑均只换字符串字面量、逻辑零改动。4 个面板已无残留中文字面量（仅 `ObjectPanel` 一处代码注释保留中文，规格 §5 允许）。`tsc·lint·build·diff --check` 全绿。

**v0.10b 第三批（light-panel/* 8 文件）已落地（Claude Code，2026-06-23；待用户验收）**：`LightPanel` + `LightBaseSection`/`LightBeamSection`/`LightColorSection`/`LightModifierSection`/`LightPositionSection`/`LightTargetSection`/`LightFixtureActions` 全部转 `useT`，统一归一到单一域文件 `messages/lightPanel.ts`（~50 key）。en/ja 同样走 **OpenRouter glm-5.2** 起草（两次调用），Claude 审校（en 归句式大小写、套用规格 §6 术语「内置器械/我的器械/自定义参数/存当前灯为器械」；日文这次无中文残留）+ 集成。**含动态状态文案**：`LightFixtureActions` 的 prompt/confirm/导入计数/导出/删除回显全部用 `t(key,{name|count})` 插值（新增 locale-aware `lightPanel.fa.statusSep`：zh `，`/en `, `/ja `、`；debug `console.info` 标签改英文）。**数据边界**：`fixtureCapabilityLabel`（全彩/双色温/暖色/白光）、`LIGHT_TYPE_LABELS`、`LIGHT_TARGET_MODE_LABELS`、fixture/modifier/color-preset `.label`、用户名一律留原样、归 v0.10.1（`LightBaseSection` 该函数上方加了注释标明）。`LightBeamSection` 的硬/柔 format、`LightColorSection` 色温（白光）·关 模板均按语法重组、逻辑不变。`tsc·lint·build·diff --check` 全绿。

**v0.10b 第四批（compare 三件套）已落地，v0.10b 全部 tier-A 抽取完成（Claude Code，2026-06-23；待用户统一验收）**：`CompareControls`/`app/compare/CompareStage` 转 `useT`，`app/compare/relativeTime.ts`（纯函数，无法用 hook）改签名为 `formatRelativeTime(ts, language)`，内部直接用纯 `t(language, 'relTime.*', {n})`，`CompareStage` 从 store 读 `language` 传入——「N 分钟前 / Nm ago / N分前」按各语语法走。新增域文件 `messages/compare.ts`（compare.* + relTime.*，24 key）；en/ja 走 OpenRouter glm-5.2 起草，Claude 审校（日文「対比→比較」按规格 §6、en 句式大小写）+ 集成。**数据边界**：`sceneDiff` 的 `d.label`/`d.hint`（C 档）、`p.name` 留原样、归 v0.10.1；`formatShortTime`（HH:MM:SS）无中文不动。`CompareControls` 的「B：」前缀/「未设置」、`CompareStage` 的「差异：」「· 同」「冻结于 {time}」及空态四段引导全部 `t(key,{...})` 化。`tsc·lint·build·diff --check` 全绿。

**v0.10b 完成小结**：tier-A 全抽完 = 对象列表 5 + PresetBar + ParamPanel 壳 + 4 独立面板 + light-panel 8 + compare 3 = **21 个文件**；`messages/` 现 12 个域文件（topBar/views/common/objectList/presetBar/paramPanel/studioPanel/cameraPanel/personPanel/objectPanel/lightPanel/compare）。全仓 `src/ui`+`src/app` 已无残留中文 UI 字符串字面量，只剩：① 代码注释；② `README.md`；③ 故意留作 v0.10.1 的数据派生标签（`fixtureCapabilityLabel` 全彩/双色温/暖色/白光 等，已加注释）。**v0.10b 已通过用户统一视觉验收（2026-06-23）。下一步**：**v0.10.1**（B 档内置数据表显示标签 id→label helper + C 档 `sceneDiff` 插值文案，合并做）和 **v0.10c/d closeout**（copy pass + 版本号升 v0.10.0 + 全套文档/README 同步，仅在用户视觉验收后）。**需 Codex 做（产品文案 + 验收，按所有权分工）**：① 对已上线 tier-A 的机器初翻 en/ja 做 copy 复核（重点：view-badge 提示语、`topBar.subtitle`、A/B 导演文案、日文自然度、对齐规格 §6 术语表）；② 为 v0.10.1 的内置数据派生标签（灯型 `LIGHT_TYPE_LABELS`、能力 全彩/双色温/暖色/白光、材质 `SCENE_OBJECT_MATERIALS`、附件 `LIGHT_MODIFIER_PRESETS`、机位 `CAMERA_PRESETS`、姿态 `POSE_PRESETS`、场景物体 `SCENE_OBJECT_PRESETS`、灯目标 `LIGHT_TARGET_MODE_LABELS`、颜色/色温预设；**品牌/型号灯具名不译**）和 C 档 `sceneDiff` 约 40 条插值文案，给权威 en/ja（可 Claude/OpenRouter 起草、Codex 复核拍板）；③ 正式确认「按风险重切」路线（规格 §5/§7：A→B(tier-A)→把内置标签/`sceneDiff` 合并进 v0.10.1）；④ 定 v0.10d 收口验收口径（切语言不改 scene/preset/fixture、EN/JA 在 1280×820 不溢出、导出 JSON 不变）并做最终验收。Claude 负责 `src/i18n/display.ts` id→label helper、`sceneDiff` 文案改造与集成。
...
- README 当时已补开源前限制说明：`MAX_LIGHTS=3`、界面语言只有简体中文、桌面优先、渲染为沟通向近似；多语言与更多光源放到开源之后。
...
- v0.6 控光线全线完成。后续 v0.7 / v0.7.1 已完成并发布，v0.8 更多光源 / 多灯管理和 v0.9 自定义灯具预设导入/导出也已完成；当前后续主线是多语言 UI。
...
- v0.6d 已完成，不再改光学公式或 gear 数值；v0.6e、v0.7/v0.7.1、v0.8 和 v0.9 也已完成。当前后续主线是多语言 UI。
...
- 用户新增路线判断：v0.7 完成后即可开源；开源第一版不等待多语言或更多光源。该路线已在 v0.7 / v0.7.1 执行完成；2026-06-22 用户与 Codex 进一步判断，过早 i18n 会拖慢新增功能字段，因此先做 v0.8 更多光源 / 多灯管理、v0.9 自定义灯具预设导入/导出；2026-06-23 多语言已收敛为 v0.10 规格。
...
- 封包/开源策略调整：不在 v0.3 后立即封包，等 v0.3-v0.6 核心功能都完成且稳定后再做 v0.7 可开源第一版；这条策略已在 v0.7 / v0.7.1 执行完成，后续路线已按顺序完成更多光源和自定义灯具预设，多语言已进入 v0.10 实现阶段（进行中）。
...
22. ✅ **[Claude Code + Codex] v0.9 自定义灯具预设导入/导出**（用户真机验收通过）：严格 JSON schema、存当前灯为器械、导入/导出、器械下拉合并、自定义预设本地持久化、删除回退和携带验证均已完成。
23. **v0.10 多语言 UI（进行中）**：按 `V0_10_I18N_SPEC.md` 做轻量 i18n，语言只作为 app preference，不能进入场景、保存方案、A/B 或自定义器械数据。a 基础设施已通过用户验收；b 核心 UI 抽取（全部 tier-A）已落地、待统一验收；剩余 v0.10.1（内置显示标签 + `sceneDiff` 文案）+ c/d 收口。
...
v0.2 多人物、v0.3 白棚结构/道具、v0.4 人物姿态、v0.4.5 承载物实时绑定、v0.4.6 A/B 引导、v0.4.7 摄影机控制、v0.4.8 store action 拆分、v0.5.0 灯具器械库、v0.5.1 渲染可信度、v0.6a 控光附件 MVP、v0.6b 附件视觉/导演简介、v0.6c 棚内独立控光器材、v0.6d 近似光学、v0.6e 收口、v0.7 开源第一版、v0.7.1 桌面图标 / Release、v0.8 多灯管理和 v0.9 自定义灯具导入/导出都已通过。不要重复做摄影机面板、store 拆分、灯具库第一版、v0.5.1 主体实现、v0.6a 附件数值、v0.6b 附件视觉、v0.6c gear 视觉、v0.6d 光学公式、v0.6e A/B 收口、v0.7 开源收口、v0.8 多灯上限或 v0.9 自定义灯具 schema/导入导出闭环。

建议先做最小闭环：

1. v0.10a 多语言基础设施：新增 `src/i18n`、语言状态、`direct-light.language.v1` 持久化和顶栏语言入口，先翻译 TopBar / view badge / 少量 common 文案。
2. v0.10b/c/d：抽取核心 UI 文案、补内置显示标签与英日 copy、做切语言不改数据和布局验收，用户通过后再升 TopBar `v0.10.0`。
...
- v0.8：更多光源 / 多灯管理。
- v0.9：自定义灯具预设导入/导出。
- v0.10：多语言 UI（进行中；a 已验收、b 核心 UI 抽取已落地待统一验收，版本号仍 v0.9.0）。
