...
Current roadmap note: v0.6, v0.7, and v0.7.1 are complete. The project is public, GitHub Pages is live, and the macOS desktop Release is published. Current recommended next work is v0.8 multilingual UI, then v0.9 more lights / multi-light management; do not reopen v0.7 unless fixing a concrete release/docs regression.
...
15. Next: v0.8 multilingual UI.
16. Then: v0.9 more lights / multi-light management.

## Working agreements (重要约定)

These are standing rules for how Codex should work on this project. Follow them by default.

1. **Delegate code-writing via OpenRouter; Codex reviews.** The user has an OpenRouter key wired through the `openrouter` MCP. For non-trivial code drafting, hand the writing to `z-ai/glm-5.2` or `qwen3.7-max` / `qwen3-coder` against a precise spec, then Codex audits line-by-line (types, edges, style), integrates, and verifies. This keeps Codex in the reviewer/integrator seat and saves the user's budget. (qwen3.7 may be blocked by an account privacy setting; fall back to glm-5.2.) See the version log in `COLLABORATION.md` for precedents.
...
3. **Hermes drafts, Codex/Codex review.** Hermes is an auxiliary code agent for bounded drafts when Codex quota is tight; product decisions stay with user + Codex, final engineering judgment with Codex/Codex. Hermes follows `HERMES.md` and hands back changed files, validation, known limits.
   ...
   - **When to use Hermes vs OpenRouter:** Both offload mechanical drafting. OpenRouter (`openrouter` MCP, glm-5.2) Codex can call *directly* in-line (no relay) but it spends Codex's turn/budget — use it when Codex has budget and wants speed/integration in one flow. Hermes runs *outside* Codex's quota (user relays) — use it specifically when Codex is near its limit and the goal is to save Codex budget. For saving Codex quota there is no more convenient path than the handoff-doc + user-relay; OpenRouter does not save Codex budget.
4. **Do not blur ownership.** If a task is marked Codex-owned, Hermes may only draft the exact subtask named in a handoff. Codex/Codex must do final integration, validation, and status updates.
