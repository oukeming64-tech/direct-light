...
- 桌面封包：用户选定 **Tauri** 并已搭好脚手架 + 发版 CI（见上方状态块第二段，`src-tauri/` + `.github/workflows/release.yml`，push `v*` tag → macOS runner 出通用 `.dmg` → Releases 草稿，未签名）。图标暂为默认占位，待后续替换。
